...
define(function(require, exports, module) {
	...
	function Index() {
		...
		this.rank = '.rank dl';
		this.rankALLPlay = '.dt-txt a.icon-play'; //飙升榜
		this.rankALLStore = '.dt-txt a.icon-store'; //飙升榜

		this.rankLI = '.rank dl dd'; //排行榜li
		this.rankLIPlay = '.dd-oper a.icon-play'; //排行榜li的播放按钮
		this.rankLIAdd = '.dd-oper a.icon-add'; //排行榜li的添加按钮
		this.rankLIStore = '.dd-oper a.icon-store'; //排行榜li的收藏按钮
	}
	...
	Index.prototype._bind = function() {
		...
		$('#hot-list').on('click', 'li', function() {
			$(self.rankALLPlay).trigger('click');
		});

		$('div.main-rank').on('click', this.rankALLPlay, function() {
			var arr = [], // 造一个歌曲的id数组
				id = 0,
